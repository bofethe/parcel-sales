...
See the provided [workflow](reports/figures/flowchart/parcel_sales.png) diagram for a visual representation.
...
To repeat this analysis, you can either use the docker container or create your own python environment based on the [requirements](requirements.txt) file.

Download the _allsales_mm_dd_yyyy_ and _parcels_mm_dd_yyyy_ zip files from [Hilsborough County Property Appraiser](https://downloads.hcpafl.org/) into the [raw](data/raw/) folder then run [injest](src/ingest.py) to preprocess the data. Next run the [EDA](notebooks/EDA.ipynb) notebook to explore the data and cleanse it for analysis. Finally, run the [analysis](notebooks/analysis.ipynb) notebook to analyze the data using various machine learning models.
